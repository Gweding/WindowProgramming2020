...
BOOL CToolApp::InitInstance()
{
	...
	m_pMainWnd->SetWindowTextW(L"Skull : The Hero Slayer - Tool");
	...
	CToolView* pView = dynamic_cast<CToolView*>(pFrameWnd->m_WndSplit.GetPane(0, 1));
	CDC* cdc = pView->GetDC();
	m_hDC = cdc->GetSafeHdc();
	g_hWnd = pView->GetSafeHwnd();
...
BOOL CToolApp::OnIdle(LONG lCount)
{
	...
	else
	{
		...
		if ( m_fTimeCnt >= 1.f )
		{
			swprintf_s(m_szFPS, L"Tool | FPS: %d", m_iFPS);
			m_pMainWnd->SetWindowTextW(m_szFPS);
			m_iFPS = 0;
			m_fTimeCnt = 0.f;
		}
	}
